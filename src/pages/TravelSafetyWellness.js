...
const TravelSafetyWellness = () => {
    return (
        <div className="safetywellness-container">
            <h1>Travel Safety & Wellness</h1>
            <div className="topics-list">
                {topics.map((topic) => (
                    <div className="topic-card" key={topic.id}>
                        ...
                        <Link to={`/safetywellness/TravelSafety/${topic.id}`} className="read-more">Read More</Link>
                    </div>
                ))}
            </div>
        </div>
...
export default TravelSafetyWellness;